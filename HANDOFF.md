...
- **Footer (TWO variants):** product/solution/category pages use the simple shared `Footer.astro`. The **homepage (`Home.astro`) + contact-sales (`ContactSales.astro`) carry a richer inline `<footer slot="footer">`** (brand blurb + Products/Solutions/Contact columns + Official Store link + `socials` icon row + **`<PaymentMethods />`**). The footer LinkedIn *text* link was removed (redundant with the `socials` LinkedIn icon). **`PaymentMethods.astro`** renders one independent SVG per brand from **`/images/payment/{visa,mastercard,amex,unionpay,paypal}.svg`** (aaronfagan `flat-rounded` colored cards; swap/add a file to change a brand; keep `astro/public/images/payment/` + `public/images/payment/` in sync). The simple `Footer.astro` shows NO payment row (extend there if payment should be site-wide).
...
- **2026-06-03 direction consistency rule:** generated product renders in the same product family must share the same direction template: same camera position, same visible side, same top/bottom receding direction, same tilt family, same baseline, and comparable subject scale. Do **not** batch-generate a product family until one direction template is approved. If direction differs, mark the asset rejected in `docs/product-image-status.json`, archive it under `asset-workbench/product-images/rejected/`, and keep it out of `astro/public/product-library/transparent/` and `public/product-library/transparent/`.
