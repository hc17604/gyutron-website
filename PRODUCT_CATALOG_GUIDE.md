...
- Cloudflare must never deploy the repository root. Use `npm run deploy` or `npx wrangler deploy --config wrangler.toml --assets ./public/` for Workers, or set Cloudflare Pages build output directory to `public`. Do not use `.` or `/opt/buildhome/repo` as the assets/output directory.
...
- Do not use flat programmatic drawings, icon-like mockups, or simplified vector-style product art for product cards. Product images must look like credible industrial catalog photography or realistic 3D product renders.
