...
function initStoreMobileMenu() {
    ...
    // NOTE: keep the /shop/ paths literal — the build rewrites them to
    // /de/shop/ , /ja/shop/ per locale. Labels go through t() / catLabel() so
    // the mobile menu is fully localized (was hardcoded English before).
    panel.innerHTML = `
        <div class="store-mobile-section">${t("Store")}</div>
        <a href="/shop/products.html">${t("nav.products")} ${chev}</a>
        <a href="/shop/request-quote.html">${t("nav.requestQuote")} ${chev}</a>
        <a href="/shop/contact-engineer.html">${t("Contact Engineer")} ${chev}</a>
        <a href="/shop/account.html">${t("Account Registration")} ${chev}</a>
        <a href="/shop/cart.html">${t("Cart")} ${chev}</a>
        <div class="store-mobile-section">${t("Product Categories")}</div>
        ${CATEGORY_META.map((category) => `<a href="/shop/products.html?category=${encodeURIComponent(category.name)}">${catLabel(category.name)} ${chev}</a>`).join("")}
        <div class="store-mobile-section">${t("Company & Policies")}</div>
        <a href="https://www.gyutron.com/">${t("nav.brandSite")} ${chev}</a>
        <a href="/shop/about-us.html">${t("About Us")} ${chev}</a>
        <a href="/shop/contact-us.html">${t("Contact Us")} ${chev}</a>
        <a href="/shop/shipping-policy.html">${t("Shipping Policy")} ${chev}</a>
        <a href="/shop/warranty-policy.html">${t("Warranty Policy")} ${chev}</a>
    `;
