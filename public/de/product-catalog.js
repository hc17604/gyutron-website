...
function createProductArt(product) {
    ...
    if (isScanner) {
        return `
            <svg viewBox="0 0 360 230" role="img" aria-label="${product.name} Produktbild">
                <defs>
...
function renderCategoryPage(categoryKey) {
    ...
    if (category.redirectTo) {
        window.location.replace(`de/${category.redirectTo}.html`);
        return;
    }
    ...
    const visibleProdukte = category.products.slice(pageStart, pageStart + PRODUCTS_PER_PAGE);
    ...
    document.querySelector("[data-section-title]").textContent = category.sectionTitle || `${category.title} Produkte`;
    document.querySelector("[data-section-intro]").textContent = category.sectionIntro || "Modellnamen, Optionen und Spezifikationen sind für das GYUTRON Portfolio strukturiert und orientieren sich an gängigen industriellen Geräteklassen.";
    ...
    document.querySelector("[data-category-nav]").innerHTML = categoryOrder.map((key) => {
        ...
        return `<a class="${key === categoryKey ? "is-active" : ""}" href="de/${key}.html">${item.navLabel}</a>`;
    }).join("");

    document.querySelector("[data-product-grid]").innerHTML = visibleProdukte.map((product) => `
        <article class="product-card">
            ...
            <div class="product-body">
                ...
                <div class="product-actions">
                    <a href="de/contact-sales.html">Angebot anfordern</a>
                    <a href="de/request-specification.html">Spezifikation vergleichen</a>
                </div>
            </div>
        </article>
    `).join("");
    ...
    document.querySelector("[data-compare-table]").innerHTML = `
        <thead><tr><th>Modell</th><th>Typ</th><th>Kernfunktion</th><th>Robustheit</th></tr></thead>
        <tbody>
...
function setupNavigation() {
    const navArtikel = Array.from(document.querySelectorAll(".nav-item"));
